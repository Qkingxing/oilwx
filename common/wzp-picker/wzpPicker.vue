<template>
	<view>
		...
		<view class="picker-content" :class="{'pickerShow':pickerShow}">
			<view class="picker-button flex justify-between">
			...
			<picker-view class="picker-view" indicator-class="picker-view-selected-one" :value="pickerIndex" @change="pickerViewChangeOne"
			 v-if="mode=='one'">
				<picker-view-column>
					<view class="picker-item" v-for="(item, index) in pickerList" :key="index">{{item.oils_name}}</view>
				</picker-view-column>
			</picker-view>
			<!-- 双列选择-联动 -->
			<picker-view class="picker-view" indicator-class="picker-view-selected-two" :value="pickerIndex" @change="pickerViewChangeTwo"
			 v-if="mode=='two_linkage'">
				<picker-view-column>
					<view class="picker-item" v-for="(item, index) in pickerList" :key="index">{{item.name}}</view>
				</picker-view-column>
				<picker-view-column>
					<view class="picker-item" v-for="(item, index) in pickerList[pickerIndex[0]].children" :key="index">{{item.name}}</view>
				</picker-view-column>
			</picker-view>
			<!-- 双列选择-不联动 -->
			<picker-view class="picker-view" indicator-class="picker-view-selected-two" :value="pickerIndex" @change="pickerViewChangeTwo"
			 v-if="mode=='two_nolinkage'">
				<picker-view-column>
					<view class="picker-item" v-for="(item, index) in pickerList.firstArray" :key="index">{{item.name}}</view>
				</picker-view-column>
				<picker-view-column>
					<view class="picker-item" v-for="(item, index) in pickerList.secondArray" :key="index">{{item.name}}</view>
				</picker-view-column>
			</picker-view>
			<!-- 三列选择-联动 -->
